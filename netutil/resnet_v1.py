...
def bottleneck(inputs,
  ...
  """
    # base_depth=16, num_units=3, stride=2
    # =>> [depth = 64, depth_bottleneck = 16, stride=1] * 2 + [depth = 64, depth_bottleneck = 16, stride=2]
    with tf.variable_scope(scope, 'bottleneck_v1', [inputs]) as sc:
        depth_in = tf_slim.layers.utils.last_dimension(inputs.get_shape(), min_rank=4)
        if depth == depth_in:
        ...
        return tf_slim.layers.utils.collect_named_outputs(outputs_collections,
                                                          sc.name,
                                                          output)
    ...
    with tf.variable_scope(
        ...
        with slim.arg_scope([slim.conv2d, bottleneck,
            ...
            if is_training is not None else NoOpScope()):
                ...
                end_points = tf_slim.layers.utils.convert_collection_to_dict(end_points_collection)
