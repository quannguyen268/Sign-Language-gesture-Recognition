...
from typing import Any, Mapping, Optional, Sequence, Tuple, Union, Dict

import tensorflow as tf
from netutil import nn_layers
...
class StreamConvBlock(ConvBlock):
  ...
  def __init__(
      ...
      use_bias: bool = False,
      kernel_initializer: tf.keras.initializers.Initializer = 'HeNormal',
      kernel_regularizer: Optional[tf.keras.regularizers.Regularizer] = tf.keras.regularizers.L2(KERNEL_WEIGHT_DECAY),
      use_batch_norm: bool = True,
      batch_norm_layer: tf.keras.layers.Layer =
      tf.keras.layers.BatchNormalization,
      batch_norm_momentum: float = 0.99,
...
class SkipBlock(tf.keras.layers.Layer):
  ...
  def call(self, inputs):
    ...
    if self._pool is not None:
      ...
      if self._conv_type == '2plus1d':
        x = tf.reshape(
            x, [tf.shape(inputs)[0], -1, tf.shape(x)[1],
